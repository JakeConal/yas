...
--version 0.45.2 --set leaderElection.enable=false \
--create-namespace --namespace kafka

kubectl wait --for=condition=Established crd/kafkas.kafka.strimzi.io --timeout=120s || true
kubectl rollout status deployment/strimzi-cluster-operator --namespace kafka --timeout=120s || true
...
kubectl rollout status statefulset/elastic-operator --namespace elasticsearch --timeout=120s || true
...
kubectl wait --for=condition=Established crd/grafanas.grafana.integreatly.org --timeout=120s || true
kubectl rollout status deployment/grafana-operator-controller-manager --namespace observability --timeout=120s || true
